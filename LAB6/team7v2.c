...
void I2C0_IRQHandler(void)
{
    ...
    if(I2C_GET_TIMEOUT_FLAG(I2C0))
        ...
        s_I2C0HandlerFn(u32Status);	 				//Keep going~~
		}
}
void I2C_status(uint32_t u32Status) 
{
		...
		else if(u32Status == 0x28)                  /* DATA has been transmitted and ACK has been received */
    {
				if(!R){
						...
						I2C_SET_CONTROL_REG(I2C0, I2C_I2CON_STA_SI); // Restart!!!
				}
    }
...
void I2C_Write(uint8_t DataAddr,uint8_t Data)
{
		g_u8MstEndFlag = 0;
		R=0;
		g_DataAddr = DataAddr; //addr at adxl 345
		g_Data = Data;				 
		I2C_SET_CONTROL_REG(I2C0, I2C_I2CON_STA);//START
		while(g_u8MstEndFlag == 0); 
}
...
void I2C0_Init(void)
{
    ...
    NVIC_EnableIRQ(I2C0_IRQn);
		...
		{
				for(i=0;i<3;i++){
						offset[i] += ((I2C_Read(0x33+(2*i))<<8) | I2C_Read(0x32+(2*i)));
...
int32_t main(void)
{
    ...
    I2C0_Init();
		...
		while(1){
			for(i=0;i<3;i++){
				axis[i]=((float)((I2C_Read(0x33+2*i) << 8) | I2C_Read(0x32+2*i))/256);//default sensitivity is (+-)2g , so we need divide 256 to get (+-)1g
			}
