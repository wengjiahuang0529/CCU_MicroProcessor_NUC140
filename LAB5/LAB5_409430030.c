...
int main(void)
{
    ...
    GPIO_ENABLE_DEBOUNCE(PA, BIT2);	
		...
		if(WDT_GET_RESET_FLAG() == 1 ) //have reset flag
				{
					g_flag=0;
					/* Use PA.2 to check time-out period time */
					GPIO_SetMode(PA, BIT2, GPIO_PMD_QUASI);
		...
		while(1){
				...
				if(g_flag==0) 
					WDT_Open(WDT_TIMEOUT_2POW16, WDT_RESET_DELAY_1026CLK, FALSE, FALSE); 
				//to change 2^16 ,WDT IRQ will wait1024*Twdt, if WDT counter not reset, WDT will generate chip reset signal.
				
				else  // have to reset
					WDT_Open(WDT_TIMEOUT_2POW16, WDT_RESET_DELAY_1026CLK, TRUE, FALSE); // will set WDT_GET_RESET_FLAG
